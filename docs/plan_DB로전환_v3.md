...
### 부하 시험 결과 (2026-08-10 · 실서버)

`tools/playtest.mjs` 로 사람 없이 돌렸다. 응답 시간은 **칸을 누르고 문제가 뜰 때까지**,
**답을 내고 결과가 뜰 때까지**를 브라우저와 같은 경로(WebSocket)로 잰 것이다.

| 규모 | 걸린 시간 | 답 → 결과 (중앙) | 최대 | 어긋남 |
|---|---|---|---|---|
| 20명 · 1개 방 | 99초 | 203ms | 285ms | 0 |
| 20명 × 3개 방 (60명) | 95초 | 201ms | 684ms | 0 |
| 20명 × 6개 방 (120명) | 93초 | 201ms | 768ms | 0 |

**방을 여섯 개 동시에 돌려도 한 개일 때와 시간이 같다.** 방 하나가 DO 하나라
서로 밀지 않는다는 설계가 숫자로 확인됐다. 중앙값은 세 경우 모두 201ms 로 붙어 있는데,
이건 서버 처리 시간이 아니라 **Cloudflare 까지 오가는 왕복 시간**이 그만큼이라는 뜻이다.
서버가 더 바빠져도 이 숫자는 잘 안 움직인다.

문제-정답 어긋남 · 재시도 중복 반영 · 순번 건너뜀 · 턴 요약 어긋남 모두 **0**.

한 반 16~20명이 목표였는데 **여섯 반이 동시에 수업해도 여유가 있다.**

**남은 것** — 수업 한 달 운영(2026-08-11~).
