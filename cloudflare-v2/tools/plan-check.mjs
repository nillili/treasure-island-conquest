 ...
 *   ③ 끝났을 때 점령률이 35~73% 안에 든다 — 헐렁하지도 마르지도 않는다
 *   ④ 17명까지는 10판, 18명부터 판수가 줄어든다
 *   ⑤ 7명은 11×11, 23명은 7판, 30명은 6판 — 사람이 많아도 6판 아래로는 안 내려간다
...
/** app.js 와 같아야 하는 값. 실제 수업 규모 10판의 중앙값이다(2026-09-05). */
const CLAIM = 0.73;
...
for (let n = 2; n <= 30; n++) {
  ...
  const fill = (n * (1 + CLAIM * p.rounds)) / cells;
  ...
  if (fill > 0.73) outOfRange.push(`${n}명 ${Math.round(fill * 100)}%`);
...
check("점령률이 73% 를 넘는 인원이 없다 — 판이 마르지 않는다", outOfRange, []);
check("어떤 인원이든 6판 아래로는 안 줄인다", tooShort, []);
check("5명부터는 점령률이 35% 아래로 안 떨어진다", tooEmpty, []);

// ④ 큰 반에서 판수가 줄어든다
const rounds = {};
for (const n of [10, 15, 17, 18, 20, 23, 26, 30]) rounds[n] = plan(n).rounds;
check("17명까지는 10판", [rounds[10], rounds[15], rounds[17]], [10, 10, 10]);
check("18명부터 판수가 줄어든다", [rounds[18] < 10, rounds[20] < 10, rounds[23] < 10], [true, true, true]);

// ⑤ 선생님이 못 박은 세 지점 (7명은 2026-09-05 수업에서 12×12 가 40% 밖에 안 차서 정했다)
check("7명은 11×11", [plan(7).rows, plan(7).cols], [11, 11]);
...
for (const n of [4, 6, 9, 10, 12, 14, 16, 17, 20, 23, 26, 30]) {
  const p = plan(n);
  const fill = (n * (1 + CLAIM * p.rounds)) / (p.rows * p.cols);
